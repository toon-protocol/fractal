...
formatting, redundancy, unclear comments, inconsistency with neighbouring code.
...
- the diff introduces a functional defect, an untested behaviour change, or
  breaks the gate
...
2. Run Fractal's gate to ensure nothing is broken — `eslint .`, `pnpm run typecheck`, `vitest run`, and `pnpm -r run build`
